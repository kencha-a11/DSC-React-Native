...
const ProductCard = React.memo<ProductCardProps>(({
  ...
  const { canRestock, canEdit, canDeduct, canRemove } = permissions;
...
export default function CashierInventoryScreen() {
  ...
  const permissions: PermissionsType = useMemo(
    () => ({
      canView: hasPermission("view_inventory"),
      canAdd: hasPermission("add_item"),
      canEdit: hasPermission("edit_items"),
      canRestock: hasPermission("restock_items"),
      canDeduct: hasPermission("deduct_items"),
      canRemove: hasPermission("remove_items"),
      canCreateCategory: hasPermission("create_categories"),
      canRemoveCategory: hasPermission("remove_categories"),
    }),
    [hasPermission]
  );
  ...
  const showToast = useCallback((message: string, type: "success" | "error" = "success") => {
    console.log("🔔 [InventoryScreen] showToast called:", message, type);
    setToast({ message, type });
    setTimeout(() => {
      console.log("🔔 [InventoryScreen] hiding toast");
      setToast(null);
    }, TOAST_DURATION);
  }, []);
  ...
  const handleRefresh = useCallback(async () => {
    console.log("🔄 [InventoryScreen] Manual refresh");
    setRefreshing(true);
    try {
      await fetchData();
  ...
  const onMutationSuccess = useCallback(async (message: string) => {
    console.log("✅ [InventoryScreen] Mutation success:", message);
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.centerContainer}>
          <MaterialIcons name="lock-outline" size={64} color="#ccc" />
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              {permissions.canAdd && activeFilterCount === 0 && (
