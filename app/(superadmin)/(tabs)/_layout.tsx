...
const TabIcon = ({ library, name, color }: { library: TabItem["library"]; name: any; color: string }) => {
...
function CustomTabBar({ state, navigation }: BottomTabBarProps) {
  ...
  const tabs: TabItem[] = [
    ...
    // { name: "report", title: "Reports", icon: "chart-bar", library: "FontAwesome5" },
  ...
  const handleTabPress = (tabName: string, isFocused: boolean) => {
    ...
    if (!event.defaultPrevented) {
      const fullPath = `/(superadmin)/(tabs)/${tabName === "index" ? "" : tabName}`;
...
const styles = StyleSheet.create({
  tabBar: {
    ...
    paddingTop: 10,
    // paddingBottom removed – now dynamic via insets
  },
  tabItem: {
    flex: 1,
